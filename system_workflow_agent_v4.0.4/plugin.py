...
try:
    ...
    from gassist_sdk.mcp import MCPClient, StdioTransport, FunctionRegistry, FunctionDef, sanitize_name, MCPError
except ImportError:
            ...
            try:
                ...
                if client.initialize():
                    ...
                    try:
                        ...
                        logger.error(f"[MCP] MCP Protocol Error listing tools for {client_name}: {e}", exc_info=True)
                    except Exception:
                        logger.error(f"[MCP] Unexpected Error listing tools for {client_name}", exc_info=True)
                    logger.info(f"[MCP] {client_name} bridge established.")
            except MCPError as e:
                logger.error(f"[MCP] {s['name']} initialization failed (Protocol Error): {e}", exc_info=True)
            except Exception:
                logger.error(f"[MCP] {s['name']} initialization failed (Unexpected Error)", exc_info=True)
