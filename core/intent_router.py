...
MAX_QUERY_LENGTH = 10_000  # Security: Restricted to 10k to prevent resource exhaustion

class IntentRouter:
    # Maximum length for user queries to prevent resource exhaustion
                    ...
                    if not calls:
                        ...
                        if not text_resp:
                            text_resp = (
                                ...
                                "1. 請嘗試更明確地描述需求。\n"
                                "2. 使用常見指令範例：\n"
                                "   1. 「系統效能診斷」：執行系統檢查、硬體監控及效能分析，並提供優化建議。\n"
                                "   2. 「Git 工作流優化」：整合 GitKraken MCP，協助處理分支管理與 Commit 流程。\n"
                                "   3. 「代碼自動重構分析」：利用 Gemini AI 進行多模態代碼審查與潛在錯誤分析。\n"
                                "   4. 「列出目前可用的工具」：探索所有已連接的 MCP 伺服器所提供的擴充工具。"
                            )
